...
def get_connection(db_path: Optional[str] = None) -> sqlite3.Connection:
    """
    Get a database connection based on DATABASE_URL env var or fallback to SQLite.
    
    This is the primary entry point for brain modules. It automatically:
    - Uses PostgreSQL if DATABASE_URL is set (production)
    - Falls back to SQLite file if db_path is provided or AOC_DB_PATH is set
    - Returns a raw connection (not wrapped in DBAdapter)
    ...
    Example:
        conn = get_connection()
        # or
        conn = get_connection("/path/to/aoc_sales.db")
    """
    import os
    ...
    if db_path is None:
        db_path = os.environ.get("AOC_DB_PATH", "aoc_sales.db")
    
    return sqlite3.connect(db_path)


def get_adapted_connection(db_path: Optional[str] = None) -> DBAdapter:
    """
    Get a database connection wrapped in DBAdapter for cross-DB compatibility.
    
    Use this when you need placeholder conversion (? -> %s) automatically.
